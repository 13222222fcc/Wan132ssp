...
-- 初始化 Rayfield
local RayfieldLibrary = Rayfield:CreateWindow({
    Name = "Wan",
    LoadingTitle = "Wan",
    LoadingSubtitle = "加载中",
    ConfigurationSaving = {
...
-- 飞车控制循环
RunService.Heartbeat:Connect(function()
    if vehicleFlyEnabled and LocalPlayer.Character then
        local Vehicle = LocalPlayer.Character:FindFirstChildWhichIsA("VehicleSeat")
        if Vehicle then
            local Direction = Vector3.new()
            ...
            if Direction.Magnitude > 0 then
                Direction = Direction.Unit * vehicleFlySpeed
            end
            
            Vehicle:ApplyImpulse(Direction)
        end
    end
end)
